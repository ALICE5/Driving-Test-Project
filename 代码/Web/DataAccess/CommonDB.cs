...
namespace DataAccess
{
    public class CommonDB : System.Collections.CollectionBase, System.IDisposable
    {
        ...
        {
            DataSet tmpDS;
            if (this.strDbType.ToUpper().Equals("SQL")) //如果是SQL语句
            {
                ...
                //strSql是SQL语句，m_StrCon是连接对象，SqlDataAdapter是SQL和DataSet的桥接器
                tmpDA.MissingSchemaAction = MissingSchemaAction.AddWithKey;

                try
                {
                    tmpDA.Fill(tmpDS, tabName);
                    //tabName指定了数据集tmpDS中的表名，可这样调用tmpDS.Tables["tabName"]
                    //也可以不需要第二个参数，若无则调用该表时用索引号，tmpDS.Tables[0]
                }
                catch (Exception ex)
                {
